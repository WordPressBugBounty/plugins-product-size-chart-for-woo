...
if ( ! class_exists( 'PSCW_PRODUCT_SIZE_CHART_F_WOO_Function' ) ) {
	class PSCW_PRODUCT_SIZE_CHART_F_WOO_Function {
		...
		public function content_data( $content_id ) {
			...
			$css = '';
			/*Inline Style*/
			wp_register_style( 'woo_table_style', false, '', PSCW_SIZE_CHART_VERSION );
			...
			if ( ! empty( $get_option['btn_horizontal'] ) ) {
				if ( 'left' === $get_option['btn_horizontal'] ) {
					$btn_horizontal = 'left:0%';
				}
				if ( 'right' === $get_option['btn_horizontal'] ) {
					$btn_horizontal = 'right:0%';
				}
			}
			...
			if ( 'columns' == $sc_cell_style ) {
				$css .= ".woo_sc_view_table.sc_id_{$content_id} tr td:nth-child(even) {background-color:{$meta_box_data['even_columns_color']};color:{$meta_box_data['even_columns_text_color']}}
				.woo_sc_view_table.sc_id_{$content_id} tr td:nth-child(odd){background-color:{$meta_box_data['odd_columns_color']};color:{$meta_box_data['odd_columns_text_color']}}";
			} else {
				$css .= ".woo_sc_view_table.sc_id_{$content_id} tr:nth-child(even) td {background-color:{$meta_box_data['odd_rows_color']};color:{$meta_box_data['odd_rows_text_color']}}
			...
			$css .= ".woo_sc_view_table.sc_id_{$content_id} th{
			...
			if ( isset( $meta_box_data['hide'] ) && 'hide_image' !== $meta_box_data['hide'] ) {
			...
			if ( isset( $meta_box_data['hide'] ) && 'hide_table' !== $meta_box_data['hide'] ) {
				if ( ! empty( $meta_box_data['table_array'] ) ) {
					$table = $meta_box_data['table_array'];
				}
				$class_table_template = $meta_box_data['table_template'] ?? '';
				if ( ! empty( $table ) ) {
					$table = json_decode( $table );
					?>
                    <div class='woo_sc_table_scroll woo_sc_table100 <?php echo esc_attr( $class_table_template ); ?>'>
