jQuery(document).ready(function ($) {
    "use strict";
    let defaulCss = {
        bgHeadColor: $('#woo_sc_head_color').val(),
        textHeadColor: $('#woo_sc_text_head_color').val(),
        bgEvenRowColor: $('#woo_sc_even_rows_color').val(),
        textEvenRowColor: $('#woo_sc_even_rows_text_color').val(),
        bgOddRowColor: $('#woo_sc_odd_rows_color').val(),
        textOddRowColor: $('#woo_sc_odd_rows_text_color').val(),
    ...
    //Css default
    function css_default( cssDefault = {} ) {
        let bgHeadColor = cssDefault.bgHeadColor ?? $('#woo_sc_head_color').val(),
            textHeadColor = cssDefault.textHeadColor ?? $('#woo_sc_text_head_color').val(),
            bgEvenRowColor = cssDefault.bgEvenRowColor ?? $('#woo_sc_even_rows_color').val(),
            textEvenRowColor = cssDefault.textEvenRowColor ?? $('#woo_sc_even_rows_text_color').val(),
            bgOddRowColor = cssDefault.bgOddRowColor ?? $('#woo_sc_odd_rows_color').val(),
            textOddRowColor = cssDefault.textOddRowColor ?? $('#woo_sc_odd_rows_text_color').val();

        let table_design = $('.woo_sc_table_design');
        table_design.find('.tr:first').find('.td').css('background-color', bgHeadColor);
        table_design.find('.tr:first').find('.td').find('input[type="text"]').css('background-color', bgHeadColor);
        $('.tr:first').find('.td').find('input[type="text"]').css('color', textHeadColor );
        let a = $('.woo_sc_table_design').find('.tr').length;
        for (let i = 0; i < a; i++) {
            if (i > 0 && i % 2 == 0) {
                table_design.find('.tr').eq(i).find('.td').css('background-color', bgEvenRowColor);
                table_design.find('.tr').eq(i).find('.td').find('input[type="text"]').css('background-color', bgEvenRowColor);
                table_design.find('.tr').eq(i).find('.td').find('input[type="text"]').css('color', textEvenRowColor);
            }
            if (i > 0 && i % 2 !== 0) {
                table_design.find('.tr').eq(i).find('.td').find('input[type="text"]').css('background-color', bgOddRowColor);
                table_design.find('.tr').eq(i).find('.td').css('background-color', bgOddRowColor);
                table_design.find('.tr').eq(i).find('.td').find('input[type="text"]').css('color', textOddRowColor);
            }
        }
    }
    ...
    $('#woo_sc_preview_btn').on('click', function () {
        if ($('#woo_sc_hide').val() !== 'hide_all') {
            if ($('#woo_sc_hide').val() !== 'hide_table') {
                ...
                if (arr.length > 0) {
                    let out = '';
                    out += `<div class="woo_sc_table100">`;
                    out += `<div class="woo_sc_table100-body"><table class='woo_sc_view_table'><thead class="woo_sc_table100-head "><tr class="woo_sc_row100 head">`;
                    for (let j = 0; j < arr[0].length; j++) {
                        out += `<th class="woo_sc_cell100">${arr[0][j]}</th>`;
            ...
            let woo_sc_table_design = $('.woo_sc_view_table');
            //Style Preview
            $('.woo_sc_table100').addClass($('#woo_sc_table_template').val());
            //head color
            woo_sc_table_design.find('th').css('background-color', $('#woo_sc_head_color').val());
            //text head color
            woo_sc_table_design.find('th').css('color', $('#woo_sc_text_head_color').val());

            //rows color text color
            if ($('input[name="woo_sc_cell_style"]:checked').val() === 'columns') {
                let wo_sc_tb_tr = woo_sc_table_design.find('tbody tr');

                wo_sc_tb_tr.each(function (i, e) {

                    let tr_this = $(this),
                        td_this_odd = tr_this.find('td:nth-child(odd)'),
                        td_this_even = tr_this.find('td:nth-child(even)');

                    td_this_odd.css({
                        'background-color': $('#woo_sc_odd_columns_color').val(),
                        'color': $('#woo_sc_odd_columns_text_color').val()
                    });
                    td_this_even.css({
                        'background-color': $('#woo_sc_even_columns_color').val(),
                        'color': $('#woo_sc_even_columns_text_color').val()
                    });
                });
...
// Remove image event
    ...
    let woo_sc_table_design = $('.woo_sc_table_design');
    ...
    /*For column*/
    $('#woo_sc_even_columns_color').wpColorPicker({
        change: function (event, ui) {
            let wo_sc_tb_tr = woo_sc_table_design.find('.tr');
            wo_sc_tb_tr.each(function (i, e) {
                if (i === 0) {
                    return true;
                }
                let tr_this = $(this),
                    td_this = tr_this.find('.td:nth-child(even)');

                td_this.css({backgroundColor: ui.color.toString()});
                td_this.find('input[type="text"]').css({backgroundColor: ui.color.toString()});
            });
        }
    });
    $('#woo_sc_even_columns_text_color').wpColorPicker({
        change: function (event, ui) {
            let wo_sc_tb_tr = woo_sc_table_design.find('.tr');
            wo_sc_tb_tr.each(function (i, e) {
                if (i === 0) {
                ...
                td_this.find('input[type="text"]').css({color: ui.color.toString()});
            });
        }
    });
    $('#woo_sc_odd_columns_color').wpColorPicker({
        change: function (event, ui) {
            let wo_sc_tb_tr = woo_sc_table_design.find('.tr');
            wo_sc_tb_tr.each(function (i, e) {
                if (i === 0) {
                ...
                let tr_this = $(this),
                    td_this = tr_this.find('.td:nth-child(odd)');
    ...
    $('#woo_sc_odd_columns_text_color').wpColorPicker({
        change: function (event, ui) {
            let wo_sc_tb_tr = woo_sc_table_design.find('.tr');
            wo_sc_tb_tr.each(function (i, e) {
                if (i === 0) {
    ...
    $('input[name=woo_sc_cell_style]').on('change', function () {
        let woo_sc_cell_style = $('input[name=woo_sc_cell_style]:checked').val();
        let old_horizontal_w = $('#woo_sc_horizontal_width').val();
        let old_vertical_w = $('#woo_sc_vertical_width').val();
        // console.log(old_horizontal_w)
        // console.log(old_vertical_w)
        $('.cell_style').hide();
        if (woo_sc_cell_style === 'rows') {
            let woo_sc_even_rows_color = $('#woo_sc_even_rows_color').val(),
                woo_sc_even_rows_text_color = $('#woo_sc_even_rows_text_color').val(),
                woo_sc_odd_rows_color = $('#woo_sc_odd_rows_color').val(),
                woo_sc_odd_rows_text_color = $('#woo_sc_odd_rows_text_color').val();
            $('#woo_sc_even_rows_color').wpColorPicker('color', woo_sc_even_rows_color);
            $('#woo_sc_even_rows_text_color').wpColorPicker('color', woo_sc_even_rows_text_color);
            $('#woo_sc_odd_rows_color').wpColorPicker('color', woo_sc_odd_rows_color);
            $('#woo_sc_odd_rows_text_color').wpColorPicker('color', woo_sc_odd_rows_text_color);
            $('.rows_styles').show();
        } else if (woo_sc_cell_style === 'columns') {
            let woo_sc_even_columns_color = $('#woo_sc_even_columns_color').val(),
                woo_sc_even_columns_text_color = $('#woo_sc_even_columns_text_color').val(),
                woo_sc_odd_columns_color = $('#woo_sc_odd_columns_color').val(),
                woo_sc_odd_columns_text_color = $('#woo_sc_odd_columns_text_color').val();
            $('#woo_sc_even_columns_color').wpColorPicker('color', woo_sc_even_columns_color);
            $('#woo_sc_even_columns_text_color').wpColorPicker('color', woo_sc_even_columns_text_color);
            $('#woo_sc_odd_columns_color').wpColorPicker('color', woo_sc_odd_columns_color);
            $('#woo_sc_odd_columns_text_color').wpColorPicker('color', woo_sc_odd_columns_text_color);
            $('.columns_styles').show();
        }
        $('#woo_sc_template').trigger('change');
        $('#woo_sc_horizontal_width').val(parseInt(old_vertical_w)).trigger('change');
        $('#woo_sc_vertical_width').val(parseInt(old_horizontal_w)).trigger('change');
    });
    ...
    function template_color(head, tex_head, even_row, text_even_row, odd_roww, text_odd_row, hor_width, hor_bor_sty, ver_width, ver_bor_sty, bor_color) {
        ...
        if (woo_sc_cell_style === 'rows') {
            ...
            $('#woo_sc_even_columns_color').val(even_row).trigger('change');
            $('#woo_sc_even_columns_text_color').val(text_even_row).trigger('change');
            $('#woo_sc_odd_columns_color').val(odd_roww).trigger('change');
            $('#woo_sc_odd_columns_text_color').val(text_odd_row).trigger('change');
        }
