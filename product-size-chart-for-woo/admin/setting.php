...
if ( ! class_exists( 'PSCW_PRODUCT_SIZE_CHART_F_WOO_Setting' ) ) {
	class PSCW_PRODUCT_SIZE_CHART_F_WOO_Setting {
		...
		public function register_scripts() {
			...
			if ( 'pscw-size-chart_page_pscw-size-chart-setting' == $current_screen ) {
				//enqueue css
				...
				//enqueue js
				wp_enqueue_script( 'size_chart_menu_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/tab.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				...
				wp_enqueue_script( 'size_chart_setting_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/woo_size_chart_setting.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				...
				wp_enqueue_script( 'size_chart_tran_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/transition.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				wp_enqueue_script( 'iris', PSCW_SIZE_CHART_PLUGIN_URL . 'js/iris.min.js', array(
					'jquery-ui-draggable',
					'jquery-ui-slider',
					'jquery-touch-punch'
				), PSCW_SIZE_CHART_VERSION, 1
				);
			}
		}
		...
		public function get_save_option() {
			...
			if ( isset( $_POST['woo_sc_save_setting'] ) ) {
				...
				if ( ! empty( $_POST['woo_sc_name'] ) ) {
					$save_option['size_chart_name'] = sanitize_text_field( wp_unslash( $_POST['woo_sc_name'] ) );
				}
                        ...
                        <table class="woo_sc_options_table form-table">
                            <tr>
                                ...
                                <td>
                                    <div class="vi-ui toggle checkbox">
                                        <input type="checkbox"
                                               name="enable" <?php if ( isset( $get_option['enable'] ) && 1 == $get_option['enable'] ) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="woo_cs_select_position" id="woo_cs_select_position"
                                        ...
                                        <option value="before_add_to_cart" <?php if ( isset( $get_option['position'] ) && 'before_add_to_cart' == $get_option['position'] ) {
											echo esc_attr( 'selected' );
										} ?>><?php esc_html_e( 'Before add to cart', 'product-size-chart-for-woo' ); ?>
                                        </option>
                                        <option value="after_add_to_cart" <?php echo esc_attr( isset( $get_option['position'] ) && 'after_add_to_cart' == $get_option['position'] ? 'selected' : '' ); ?>>
											<?php esc_html_e( 'After add to cart', 'product-size-chart-for-woo' ); ?>
                                        </option>
                                        <option value="pop-up" <?php if ( isset( $get_option['position'] ) && 'pop-up' == $get_option['position'] ) {
											echo esc_attr( 'selected' );
										} ?>><?php esc_html_e( 'Pop-up', 'product-size-chart-for-woo' ) ?>
                                        </option>
                                        <option value="product_tabs" <?php if ( isset( $get_option['position'] ) && 'product_tabs' == $get_option['position'] ) {
											echo esc_attr( 'selected' );
										} ?>><?php esc_html_e( 'Product tab', 'product-size-chart-for-woo' ) ?>
                                        </option>
                                        <option value="none" <?php if ( isset( $get_option['position'] ) && 'none' === $get_option['position'] ) {
											echo esc_attr( 'selected' );
                            ...
                            <tr class="woo_sc_multi" style="display: none">
                                ...
                                <td>
                                    <div class="vi-ui toggle checkbox">
                                        <input type="checkbox"
                                               name="woo_sc_multi_sc" <?php if ( isset( $get_option['multi_sc'] ) && 1 == $get_option['multi_sc'] ) {
											echo esc_attr( 'checked' );
										} ?>>
                                        <label></label>
                                    </div>
                            ...
                            <tr class="woo_sc_btn_popup_position" style="display: none">
                                ...
                                <td>
                                    <select name="woo_sc_btn_horizontal" class="vi-ui dropdown setting_field ">
                                        <option value="right" <?php if ( isset( $get_option['btn_horizontal'] ) && "right" === $get_option['btn_horizontal'] ) {
											echo esc_attr( "selected" );
										} ?>>
											<?php esc_html_e( 'Right', 'product-size-chart-for-woo' ) ?></option>
                                        <option value="left" <?php if ( isset( $get_option['btn_horizontal'] ) && "left" === $get_option['btn_horizontal'] ) {
											echo esc_attr( "selected" );
										} ?>>
                            ...
                            <tr class="woo_sc_btn_type" style="display: none">
                                ...
                                <td>
                                    <select name="woo_sc_button_type" class="vi-ui dropdown setting_field"
                                            id="woo_sc_type_btn">
                                        <option value="icon" <?php if ( ! empty( $get_option['button_type'] ) && "icon" === $get_option['button_type'] ) {
											echo esc_attr( "selected" );
										} ?>>
											<?php esc_html_e( 'Icon', 'product-size-chart-for-woo' ); ?></option>
                                        <option value="text" <?php if ( ! empty( $get_option['button_type'] ) && "text" === $get_option['button_type'] ) {
											echo esc_attr( "selected" );
