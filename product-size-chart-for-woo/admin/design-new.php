...
if ( ! class_exists( 'PSCW_PRODUCT_SIZE_CHART_F_WOO_Design' ) ) {
	class PSCW_PRODUCT_SIZE_CHART_F_WOO_Design {
		function __construct() {
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'register_scripts' ),999999 );
			add_action( 'admin_enqueue_scripts', array( $this, 'data_transmission' ),9999999 );
		...
		public function register_scripts() {
			...
			if ( ( 'pscw-size-chart' == $current_screen ) || ( 'edit-pscw-size-chart' == $current_screen ) ) {
				wp_enqueue_script( 'size_chart_dimmer_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/dimmer.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				wp_enqueue_script( 'size_chart_transition_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/transition.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				wp_enqueue_script( 'size_chart_checkbox_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/checkbox.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
			...
			if ( 'pscw-size-chart' == $current_screen ) {
				wp_enqueue_script( 'size_chart_modal_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/vi-modal.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );

				wp_enqueue_style( 'size_chart_modal_css', PSCW_SIZE_CHART_PLUGIN_URL . 'css/vi-modal.min.css', '', PSCW_SIZE_CHART_VERSION );
				wp_enqueue_style( 'size_chart_select2_css', PSCW_SIZE_CHART_PLUGIN_URL . 'css/select2.min.css', '', PSCW_SIZE_CHART_VERSION );
				if ( WP_DEBUG ) {
					wp_enqueue_style( 'size_chart_back-end_css', PSCW_SIZE_CHART_PLUGIN_URL . 'css/woo_size_chart_back_end.css', '', PSCW_SIZE_CHART_VERSION );
					wp_enqueue_script( 'woo_size_chart_design_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/woo_size_chart_design.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				} else {
					...
					wp_enqueue_script( 'woo_size_chart_design_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/woo_size_chart_design.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				}

				wp_enqueue_script( 'size_chart_select2_js', PSCW_SIZE_CHART_PLUGIN_URL . '/js/select2.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				...
				wp_enqueue_script( 'size_chart_form_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/form.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
				wp_enqueue_script( 'size_chart_dropdown_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/dropdown.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );

			}
			if ( 'edit-pscw-size-chart' == $current_screen ) {
				...
				wp_enqueue_script( 'size_chart_edit_woo_sc_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/woo_size_chart_edit_woo_size_chart.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION, true );
		...
		public function duplicate_template() {
            ...
            if ( isset( $_GET['scnonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_GET['scnonce'] ) ), 'woo_sc_nonce' ) ) {
		...
		public function post_add_action( $actions, $post ) {
			if ( "pscw-size-chart" == $post->post_type ) {
                $nonce   = wp_create_nonce('woo_sc_nonce' );
				$href    = admin_url( "post.php?action=pscfw_duplicate&id={$post->ID}&scnonce={$nonce}" );
                    ...
                    <div class="woo_sc_img_upload">
						<?php if ( ! empty( $meta_box_data['img_link'] ) ) {
							...
							?><img class="image_Default" style="width:95%;display:inline-block;"
                                   src="<?php echo esc_url( PSCW_SIZE_CHART_PLUGIN_URL . 'img/Photos-new-icon.png' ); ?>">
							<?php
						} ?>
                    </div>
                </div>
		...
		public function woo_sc_size_chart_design( $post_id ) {
			...
			$args_table_template = [
				'table_template_v0' => 'Default',
				'table_template_v1' => 'Template v1',
				'table_template_v2' => 'Template v2',
			];

			$sc_table_template = $meta_box_data['table_template'] ?? '';
			$sc_template       = $meta_box_data['template'] ?? '';
			$sc_cell_style     = $meta_box_data['woo_sc_cell_style'] ?? 'rows';
			$sc_table_hide     = $meta_box_data['hide'] ?? 'none';
			?>
            <h3><?php esc_html_e( 'Tools', 'product-size-chart-for-woo' ) ?></h3>
            <input type="hidden" name="prevent_delete_meta_movetotrash" id="prevent_delete_meta_movetotrash" value="<?php echo esc_attr( wp_create_nonce( plugin_basename( __FILE__ ) . $post_id->ID ) ); ?>">
            <table class="woo_sc_table_tools vi-ui form">
                <tr>
                    <td>
						<?php esc_html_e( 'Table Template', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                        <select name="woo_sc_table_template" id="woo_sc_table_template" class="vi-ui dropdown woo_sc_table_tools">
                            <option value=""><?php esc_html_e( 'Select template', 'product-size-chart-for-woo' ) ?></option>
							<?php
							foreach ( $args_table_template as $key_table_temp => $table_temp ) {
								?>
                                <option value="<?php echo esc_attr( $key_table_temp ); ?>" <?php selected( $sc_table_template, $key_table_temp ); ?>><?php echo esc_html( $table_temp ) ?></option>
								<?php
							}
							?>
                        </select>
                ...
                <tr>
                    <td>
						<?php esc_html_e( 'Template color', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                        <select name="woo_sc_template" id="woo_sc_template" class="vi-ui dropdown woo_sc_table_tools">
                            <option value=""><?php esc_html_e( 'Select template', 'product-size-chart-for-woo' ) ?></option>
							<?php
							foreach ( $args_template as $key_temp => $temp ) {
								?>
                                <option value="<?php echo esc_attr( $key_temp ); ?>" <?php selected( $sc_template, $key_temp ); ?>><?php echo esc_html( $temp ) ?></option>
								<?php
							}
							?>
                        </select>
                ...
                <tr>
                    ...
                    <td>
                        <select name="woo_sc_hide" id="woo_sc_hide" class="vi-ui dropdown woo_sc_table_tools">
                            <option value="none" <?php selected( $sc_table_hide, 'none' ); ?>><?php esc_html_e( 'None', 'product-size-chart-for-woo' ) ?></option>
                            <option value="hide_table" <?php selected( $sc_table_hide, 'hide_table' ); ?>><?php esc_html_e( 'Table', 'product-size-chart-for-woo' ) ?></option>
                            <option value="hide_image" <?php selected( $sc_table_hide, 'hide_image' ); ?>><?php esc_html_e( 'Image', 'product-size-chart-for-woo' ) ?></option>
                            <option value="hide_all" <?php selected( $sc_table_hide, 'hide_all' ); ?>><?php esc_html_e( 'Hide all', 'product-size-chart-for-woo' ) ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
            ...
            <table class="woo_sc_style_table vi-ui form-table">
                ...
                <tr>
                    <td>
						<?php esc_html_e( 'Style even/odd', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                            ...
                            <div class="grouped fields">
                                <div class="field">
                                    <div class="vi-ui slider checkbox">
                                        <input type="radio" name="woo_sc_cell_style" value="rows" <?php checked( $sc_cell_style, 'rows' ) ?>>
                                        <label>Rows</label>
                                    </div>
                                </div>
                                <div class="field">
                                    <div class="vi-ui slider checkbox">
                                        <input type="radio" name="woo_sc_cell_style" value="columns" <?php checked( $sc_cell_style, 'columns' ) ?>>
                                        <label>Columns</label>
                                    </div>
                                </div>
                            </div>
                ...
                <!--Row odd/even-->
                <tr class="cell_style rows_styles">
                    <td>
                ...
                <!--Column odd/even-->
                <tr class="cell_style columns_styles">
                    <td>
						<?php esc_html_e( 'Even columns background', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                        <input type="text" class="color_picker" name="woo_sc_even_columns_color"
                               id="woo_sc_even_columns_color"
                               value="<?php if ( ! empty( $meta_box_data['even_columns_color'] ) ) {
							       echo esc_attr( $meta_box_data['even_columns_color'] );
						       } else {
							       echo esc_attr( '#cceafc' );
						       } ?>">
                    </td>
                    <td>
						<?php esc_html_e( 'Even columns text', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                        <input type="text" class="color_picker" name="woo_sc_even_columns_text_color"
                               id="woo_sc_even_columns_text_color"
                               value="<?php if ( ! empty( $meta_box_data['even_columns_text_color'] ) ) {
							       echo esc_attr( $meta_box_data['even_columns_text_color'] );
						       } else {
							       echo esc_attr( '#000000' );
						       }
						       ?>">
                ...
                <tr class="cell_style columns_styles">
                    <td>
						<?php esc_html_e( 'Odd columns background', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                        <input type="text" class="color_picker" name="woo_sc_odd_columns_color"
                               id="woo_sc_odd_columns_color"
                               value="<?php if ( ! empty( $meta_box_data['odd_columns_color'] ) ) {
							       echo esc_attr( $meta_box_data['odd_columns_color'] );
						       } else {
							       echo esc_attr( '#ffffff' );
						       } ?>">
                    </td>
                    <td>
						<?php esc_html_e( 'Odd columns text', 'product-size-chart-for-woo' ) ?>
                    </td>
                    <td>
                        <input type="text" class="color_picker" name="woo_sc_odd_columns_text_color"
                               id="woo_sc_odd_columns_text_color"
                               value="<?php if ( ! empty( $meta_box_data['odd_columns_text_color'] ) ) {
							       echo esc_attr( $meta_box_data['odd_columns_text_color'] );
						       } else {
							       echo esc_attr( '#000000' );
						       } ?>">
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <select name="woo_sc_horizontal_border_style" id="woo_sc_horizontal_border_style"
                            ...
                            <option value="solid"
								<?php if ( ! empty( $meta_box_data['horizontal_border_style'] ) && 'solid' == $meta_box_data['horizontal_border_style'] ) {
									echo esc_attr( 'selected' );
								} ?>><?php esc_html_e( 'Solid', 'product-size-chart-for-woo' ) ?>
                            </option>
                            <option value="dashed"
								<?php if ( ! empty( $meta_box_data['horizontal_border_style'] ) && 'dashed' == $meta_box_data['horizontal_border_style'] ) {
									echo esc_attr( 'selected' );
								} ?>><?php esc_html_e( 'Dashed', 'product-size-chart-for-woo' ) ?>
                            </option>
                            <option value="dotted"
								<?php if ( ! empty( $meta_box_data['horizontal_border_style'] ) && 'dotted' == $meta_box_data['horizontal_border_style'] ) {
									echo esc_attr( 'selected' );
                ...
                <tr>
                    ...
                    <td>
                        <select name="woo_sc_vertical_border_style" id="woo_sc_vertical_border_style"
                                class="vi-ui dropdown border_field">
                            <option value="solid"
								<?php if ( ! empty( $meta_box_data['vertical_border_style'] ) && 'solid' == $meta_box_data['vertical_border_style'] ) {
									echo esc_attr( 'selected' );
								} ?>><?php esc_html_e( 'Solid', 'product-size-chart-for-woo' ) ?>
                            </option>
                            <option value="dashed"
								<?php if ( ! empty( $meta_box_data['vertical_border_style'] ) && 'dashed' == $meta_box_data['vertical_border_style'] ) {
									echo esc_attr( 'selected' );
								} ?>><?php esc_html_e( 'Dashed', 'product-size-chart-for-woo' ) ?>
                            </option>
                            <option value="dotted"
								<?php if ( ! empty( $meta_box_data['vertical_border_style'] ) && 'dotted' == $meta_box_data['vertical_border_style'] ) {
									echo esc_attr( 'selected' );
								} ?>><?php esc_html_e( 'Dotted', 'product-size-chart-for-woo' ) ?>
                            </option>
                        </select>
		...
		public function assign_categories_callback( $post_id ) {
			...
			if ( ! empty( $meta_box_data['categories'] ) ) {
				$load_save_categories = $meta_box_data['categories'];
			}
                ...
                <div class="assign_cate_scroll">
                    <ul>
						...
						foreach ( $cat_tree as $item ) {
							if ( 'child_0' === $item->data_name ) {
								?>
                                <li class="item_level_<?php echo esc_attr( $item->level ) . ' ' . esc_attr( $item->data_name ); ?>"
                                    style="margin-left:<?php echo esc_attr( ( $item->level = $item->level * 20 ) . 'px' ); ?>">
                                    <input type="checkbox" value="<?php echo esc_attr( $item->term_id ) ?>"
										...
										<?php
										if ( ! empty( $load_save_categories ) ) {
											foreach ( $load_save_categories as $category ) {
												if ( ! empty( $load_save_categories ) ) {
		...
		public function woo_sc_ajax_search() {
            if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_POST['nonce'] ) ), 'woo_sc_nonce' ) ) {
		...
		function save_data_meta_box( $post_id ) {
			...
			$meta_box_even_columns_color_value      = "";
			$meta_box_even_columns_text_color_value = "";
			$meta_box_odd_columns_color_value       = "";
			$meta_box_odd_columns_text_color_value  = "";
			...
			$meta_box_hide                          = "";
			$meta_box_img_width                     = "";
			$woo_sc_cell_style                      = "rows";

			if ( isset( $_POST['check_box_select_cate'] ) ) {
				$meta_box_select_categories_value = array_map( 'sanitize_text_field', wp_unslash( $_POST['check_box_select_cate'] ) );
			}
			if ( isset( $_POST['woo_sc_table_template'] ) ) {
				$meta_box_table_template_value = sanitize_text_field( wp_unslash( $_POST['woo_sc_table_template'] ) );
			}
			if ( isset( $_POST['woo_sc_template'] ) ) {
				$meta_box_template_value = sanitize_text_field( wp_unslash( $_POST['woo_sc_template'] ) );
			}
			if ( isset( $_POST["woo_sc_head_color"] ) ) {
				$meta_box_head_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_head_color"] ) );
			}
			if ( isset( $_POST["woo_sc_text_head_color"] ) ) {
				$meta_box_text_head_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_text_head_color"] ) );
			}
			if ( isset( $_POST["woo_sc_even_columns_color"] ) ) {
				$meta_box_even_columns_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_even_columns_color"] ) );
			}
			if ( isset( $_POST["woo_sc_cell_style"] ) ) {
				$woo_sc_cell_style = sanitize_text_field( wp_unslash( $_POST["woo_sc_cell_style"] ) );
			}
			if ( isset( $_POST["woo_sc_even_rows_color"] ) ) {
				$meta_box_even_rows_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_even_rows_color"] ) );
			}
			if ( isset( $_POST["woo_sc_odd_columns_color"] ) ) {
				$meta_box_odd_columns_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_odd_columns_color"] ) );
			}
			if ( isset( $_POST["woo_sc_odd_rows_color"] ) ) {
				$meta_box_odd_rows_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_odd_rows_color"] ) );
			}
			if ( isset( $_POST["woo_sc_odd_rows_text_color"] ) ) {
				$meta_box_odd_rows_text_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_odd_rows_text_color"] ) );
			}
			if ( isset( $_POST["woo_sc_odd_columns_text_color"] ) ) {
				$meta_box_odd_columns_text_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_odd_columns_text_color"] ) );
			}
			if ( isset( $_POST["woo_sc_even_rows_text_color"] ) ) {
				$meta_box_even_rows_text_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_even_rows_text_color"] ) );
			}
			if ( isset( $_POST["woo_sc_even_columns_text_color"] ) ) {
				$meta_box_even_columns_text_color_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_even_columns_text_color"] ) );
			}
			//border style
			if ( isset( $_POST["woo_sc_horizontal_width"] ) && is_numeric( $_POST["woo_sc_horizontal_width"] ) ) {
				$meta_box_horizontal_width_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_horizontal_width"] ) );
			}
			if ( isset( $_POST["woo_sc_vertical_width"] ) && is_numeric( $_POST["woo_sc_vertical_width"] ) ) {
				$meta_box_vertical_width_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_vertical_width"] ) );
			}
			if ( isset( $_POST["woo_sc_horizontal_border_style"] ) ) {
				$meta_box_horizontal_border_style_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_horizontal_border_style"] ) );
			}
			if ( isset( $_POST["woo_sc_vertical_border_style"] ) ) {
				$meta_box_vertical_border_style_value = sanitize_text_field( wp_unslash( $_POST["woo_sc_vertical_border_style"] ) );
			}
			if ( isset( $_POST["border_color"] ) ) {
				$meta_box_border_color_value = sanitize_text_field( wp_unslash( $_POST["border_color"] ) );
			}
			if ( isset( $_POST["tableArray"] ) ) {
				$meta_box_table_array_value = sanitize_text_field( wp_unslash( $_POST["tableArray"] ) );
			}
			if ( isset( $_POST["imgLink"] ) ) {
				$meta_box_img_link_value = sanitize_text_field( wp_unslash( $_POST["imgLink"] ) );
			}
			if ( isset( $_POST['sc_input_search_product'] ) ) {
				$meta_box_search_product = array_map( 'sanitize_text_field', wp_unslash( $_POST['sc_input_search_product'] ) );
			}
			if ( isset( $_POST['woo_sc_hide'] ) ) {
				$meta_box_hide = sanitize_text_field( wp_unslash( $_POST['woo_sc_hide'] ) );
			}
			if ( ! empty( $_POST['woo_sc_width_img'] ) && $_POST['woo_sc_width_img'] > 0 && $_POST['woo_sc_width_img'] <= 100 ) {
				$meta_box_img_width = sanitize_text_field( wp_unslash( $_POST['woo_sc_width_img'] ) );
			}
			...
			$meta_box_data = [
				...
				'table_template'          => $meta_box_table_template_value,
				...
				'even_columns_color'      => $meta_box_even_columns_color_value,
				'even_rows_text_color'    => $meta_box_even_rows_text_color_value,
				'even_columns_text_color' => $meta_box_even_columns_text_color_value,
				'odd_rows_color'          => $meta_box_odd_rows_color_value,
				'odd_columns_color'       => $meta_box_odd_columns_color_value,
				'odd_rows_text_color'     => $meta_box_odd_rows_text_color_value,
				'odd_columns_text_color'  => $meta_box_odd_columns_text_color_value,
				...
				'woo_sc_cell_style'       => $woo_sc_cell_style

			];
			update_post_meta( $post_id, "woo_sc_size_chart_data", $meta_box_data );
		}

		public function data_transmission() {
			$tran_arr = array(
		...
		public function short_code_content( $atts ) {
			...
			wp_register_style( 'woo_table_style', false, array(), PSCW_SIZE_CHART_VERSION );
